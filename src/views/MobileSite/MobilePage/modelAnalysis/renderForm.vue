<template>
  <div class="listCard">
    ...
    <van-form :show-error="false" :show-error-message="false" ref="renderForm">
      <div
        class="cells"
      ...
      >
        ...
        >
          <template
            ...
            v-if="filedsValue[`$${item.modelAndField}`]"
          >
            <van-tag
        ...
        >
          <template #input v-if="filedsValue[`$${item.modelAndField}`]">
            <van-tag
              v-for="(e, i) in filedsValue[`$${item.modelAndField}`].split(',')"
              :key="i"
              plain
...
import checkboxSelect from "../../MobilePage/FlowDetail/component/checkboxSelect";
import radioSelect from "../../MobilePage/FlowDetail/component/radioSelect";
import selectPicker from "../../MobilePage/FlowDetail/component/selectPicker";
import mutiSelect from "../../MobilePage/FlowDetail/component/mutiSelect";
import fuzzySearch from "../../MobilePage/FlowDetail/component/fuzzySearch";
import userSearch from "../../MobilePage/FlowDetail/component/userSearch";
import deptSearch from "../../MobilePage/FlowDetail/component/deptSearch";
...
export default {
  ...
  components: {
    ...
    dynamicTable,
    calanderTimePicker: () =>
      import("../../MobilePage/FlowDetail/component/calanderTimePicker"),
  },
  data() {
    return {
      renderLoading: false,
  ...
  methods: {
    async queryFormFileds() {
      let { formId, formType, dataId } = this.renderFormInfo;
      ...
      const { data } = await queryForm(formId, formType, "", "");
      if (data.code === 200) {
        this.renderLoading = false;
        this.showtable = true;
        this.mainModel = data.data.mainModel;

        this.mainModelId = data.data.id;

        this.dbTableName = data.data.dbTableName;

        addDefaultValue(data.data).then((resp) => {
          this.filedsValue = resp.data.data;
        });
        ...
        this.mainModel.forEach((e) => {
          this.mainModelShow.push({
            ...e,
            display: e.addDisplay,
            readonly: e.addDisabled,
            required: e.addRequired,
            align: _this.filedsAlign(e, e.addDisabled),
    ...
    setValue(val, item) {
      this.$set(this.filedsValue, item.modelAndField, val);
    },
    ...
    handleSubmitForm(item) {
      if (item.triggerConditions == 6) {
        if (this.filedsValue.ID) {
          console.log('编辑保存');
        } else {
          // this.addSave();
          console.log('新增保存');
    ...
    addSave() {
      this.$refs.renderForm
        .validate()
        .then((_) => {
          let from = this.$refs.renderForm.getValues();
          add({
      ...
      this.$refs.renderForm
        ...
        .then((_) => {
          ...
          add({
            ...
            if (res.data.code == 200) {
              Toast.success("新增成功");
              this.filedsValue.ID = res.data.data.id;
              // 启动流程
              startFlow({
                flowKey: flowKey,
                dataId: this.mainModelId,
                ...
                variables: {
                  functionPath: this.$route.query.path,
                },
              }).then((res1) => {
