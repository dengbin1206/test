    ...
    <div class="footer">
      <div class="people">
        <van-tag
          ...
          class="tag"
        >
          {{ `${allList.has(item) ? allList.get(item).deptName : item}` }}
        </van-tag>
      </div>
...
<script>
import { deptList } from "@/api/mobileApi/flow";
...
export default {
  ...
  methods: {
    ...
    async init() {
      if (this.resultRecall.length > 0) {
        const { data } = await deptList({
          search: this.keyWords,
          current: this.current,
          size: -1,
          deptIds: this.resultRecall,
        });
    ...
    onConfirm() {
      ...
      this.result.forEach((e) => {
        this.returnLabelList.push(this.allList.get(e).deptName);
