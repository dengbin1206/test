<template>
  <div class="copyList">
    <van-nav-bar
      ...
      fixed
      placeholder
      @click-left="onConfirm"
    />

    <div class="searchBox">
      <div class="search-content">
        <van-icon name="search" style="margin-top: 2px; margin-right: 2px" />
        <input type="text" placeholder="请输入搜索关键词" @input="search" />
      </div>
    </div>
              ...
              <template #title>
                ...
                >
                  <template #default>
                    ...
                    <div style="font-size: 12px; color: #bcbdc1">
                      {{ item.deptName }}
                    </div>
                  </template>
                </van-checkbox>
    ...
    <div class="footer">
      <div class="people">
        <van-tag
          ...
          class="tag"
        >
          {{ `${allList.get(item).realName}` }}
        </van-tag>
      </div>
      <div class="customBtn" v-if="resultRecall.length > 0" @click="onConfirm">
...
export default {
  ...
  watch: {
    // 监听 store 中的 selectedList,如果有值就把最新的值遍历放进结果集返显选中
    // 否则就置空，如果用户不点击确定就不保存，保证每次进入选人时为空
    selectedList(next, pre) {
      this.result = [];
      this.resultRecall = [];
      if (next.length > 0) {
        this.selectedList.map((e) => {
          this.result.push(e.id);
          this.allList.set(e.id, e);
          this.resultRecall.push(e.id);
        });
      }
    },
  },
  ...
  methods: {
    ...
    onConfirm() {
      // resaveList 作用 resultRecall 中存储的是选中用户的 id 将 id 遍历 从 alllist Map 中 get出完整 对象返回
      this.resaveList =
        this.resultRecall &&
        this.resultRecall.map((e) => {
          return this.allList.get(e);
        });

      // 将完整 list 存储 到 vuex
      this.SET_SELECTEDLIST(this.resaveList);

      this.$emit("onConfirm", this.resaveList);
