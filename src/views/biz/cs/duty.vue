...
import addDialog from "./components/dutyAdd";
import { list,deletedById } from "@/api/biz/cs/duty";
...
export default {
  ...
  methods: {
    ...
    open() {
      ...
      this.bar='新增 责任认定'
      setTimeout(() => {
        this.$refs.addLog.open(this.rowId,this.zrId,1);
      }, 10);
    },
    edit(row) {
      this.dialogVisible=true
      this.bar='编辑 责任认定'
      setTimeout(() => {
        this.$refs.addLog.open(row.id,this.zrId,2);
      }, 10);
    },
    look(row) {
      this.dialogVisible=true
      this.bar='查看 责任认定'
      setTimeout(() => {
        this.$refs.addLog.open(row.id,this.zrId,3);
      }, 10);
    },
    turnoff(){
  ...
  activated() {
    this.api();
    if(localStorage.getItem('zrrd')){
      this.zrId=JSON.parse(localStorage.getItem('zrrd')).zrId
      this.open()
    };
  },
  data() {
        ...
        rows: [
          ...
          [
            ...
            },
            {
              ...
              label: "责任单号:",
              placeholder: "请输入关键字",
              url:'api/blade-cs/blame-confirm/oddNumByLabel?search='
            }
          ],
        ]
      },
