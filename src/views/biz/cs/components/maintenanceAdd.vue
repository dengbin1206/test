    ...
    <div style="width:100%;padding:0 20px;">
        ...
        <el-form :model="addList" :label-position='labelPosition' :rules="rules" ref="ruleForm" label-width="150px"   class="sx_formItem">
            ...
            <el-row>
                ...
                <el-table
                    :data="addList.maintenanceProductInfoVOS"
                    ...
                    <el-table-column  prop="finishNum" label="维保完成数量"></el-table-column>
                </el-table>
            </el-row>
...
import {save,detail} from '@/api/biz/cs/maintenance'
...
export default {
  ...
  data() {
    return {
      ...
      dataList: [],
      dataSrcId: "CS_WBSQ",
      affixType: "CS_WBSQ#AFFIX_UPLOAD",
      mainId: "",
      rules: {
  ...
  methods: {
    ...
    contractIdChange(){
        ...
        this.contractIds.forEach(item=>{
            if(item.paperContractNo==this.addList.paperContractNo){
                ...
                this.$set(this.addList,'maintenanceProductInfoVOS',item.productContractInfoVOS)
                this.addList.maintenanceProductInfoVOS.forEach((item,index)=>{
    ...
    getDefaultVal(){
        addDefaultVaule('CS_WBSQ').then(res=>{
    ...
    pgxxOpen(){
      this.pgxxDia=true
      setTimeout(() => {
        this.$refs.pgxx.confirmProcess(this.addList.id,2)
      }, 1000)
    },
    bnfcOpen(){
      this.turnoff()
