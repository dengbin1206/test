...
import addDialog from "./components/factoryAdd";
import { list,deletedById } from "@/api/biz/cs/factory";
...
export default {
  ...
  methods: {
    ...
    open() {
      ...
      this.bar='新增 返厂'
      setTimeout(() => {
        this.$refs.addLog.open(this.rowId,this.fcId,1);
      }, 10);
    },
    edit(row) {
      this.dialogVisible=true
      this.bar='编辑 返厂'
      setTimeout(() => {
        this.$refs.addLog.open(row.id,this.fcId,2);
      }, 10);
    },
    look(row) {
      this.dialogVisible=true
      this.bar='查看 返厂'
      setTimeout(() => {
        this.$refs.addLog.open(row.id,this.fcId,3);
      }, 10);
    },
    turnoff(){
  ...
  activated() {
    this.api();
    if(localStorage.getItem('bnfc')){
      this.fcId=JSON.parse(localStorage.getItem('bnfc')).fcId
      this.open()
    };
  },
  data() {
