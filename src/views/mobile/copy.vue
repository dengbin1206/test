...
<el-container style="background:#eee;width:100%;height:100%">
        <el-header>
             <el-row style="width:100%">
                    ...
                    <el-col span="5" style="text-align:center;line-height:60px">抄送回复</el-col>
                </el-row>
        </el-header>
        <el-header style="background:#fff;border-bottom:1px solid #ccc;height:40px;line-height:40px;padding:0;">
            <div style="height:100%;border-bottom:1px solid #ccc;padding:0px;text-align:center">
              <el-button-group style="width:100%;padding:0" class="sx_btns_color">
        ...
        <el-main>
            <ul class="list" v-infinite-scroll="load" infinite-scroll-disabled="disabled" infinite-scroll-distance='10' infinite-scroll-immediate='false' style="width:100%;overflow:visible">
                <el-row style="width:100%;border-bottom:1px solid #ccc;background:#fff;" v-for="(item,index) in dataList" :key='index' class="list-item" v-on:click.native="detail(item,4)">
                    <el-col :span="15">
                    ...
                    <el-col :span="9" style="display:flex">
                        <span  style="margin-right:5px;font-size:12px;margin-top:29px">
                            <el-tag type="warning" size="mini" v-if="item.readStatus === 0">未读</el-tag>
                            <el-tag type="success" size="mini" v-if="item.readStatus === 1">已读</el-tag>
                        </span>
                        <span style="margin-right:10px;font-size:12px;">
                            <p style="margin:7px 0px;height:15px;width:65px;margin-left:-10px;white-space:nowrap;text-overflow:ellipsis;">{{item.createTime}}</p>
                            <el-tag type="warning" size="mini" v-if="item.status === 1">处理中</el-tag>
                            <el-tag type="success" size="mini" v-if="item.status === 2">已完成</el-tag>
                            <el-tag type="danger" size="mini" v-if="item.status === 3">已作废</el-tag>
                        </span>
                        <span style='line-height:60px;margin-right:10px'><i class="el-icon-arrow-right" style="font-size:20px"></i></span>
                    </el-col>
                </el-row>
            </ul>
...
<script>
import { instCopyReply,changeCopyStatus } from "@/api/mobile";
export default {
  data() {
    return {
      inp: "",
  ...
  methods: {
    ...
    search(a,b,c,d) {
      this.arr=[]
      this.current=c
      instCopyReply(a,b,c,d).then(res => {
    ...
    load() {
      ...
      if (this.arr.indexOf(this.current) < 0) {
        ...
        this.loading = true;
        setTimeout(() => {
          instCopyReply(this.inp,this.old,this.current, this.size).then(res => {
            if (res.data.code == "200") {
    ...
    detail(item,type){
         this.$router.push({path:'/mobile/todoDet',query:{proInstId:item.proInstId,taskId:'',type:type,status:item.status,id:item.id}})
         if(type==4&&item.readStatus==0){
           changeCopyStatus(item.proInstId).then(res=>{
           })
         }
    }
  },
