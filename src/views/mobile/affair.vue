    ...
    <el-container style="background:#eee;width:100%;height:100%">
        <el-header>
             <el-row style="width:100%">
                    ...
                    <el-col span="5" style="text-align:center;line-height:60px">我的事务</el-col>
                </el-row>
        </el-header>
        <el-header style="background:#fff;border-bottom:1px solid #ccc;height:40px;line-height:40px;padding:0;">
            <div style="height:100%;border-bottom:1px solid #ccc;padding:0px;text-align:center;width:100%">
              <el-button-group style="width:100%;padding:0" class="sx_affair_btns">
        ...
        <el-main>
            <ul class="list" v-infinite-scroll="load" infinite-scroll-disabled="disabled" infinite-scroll-distance='10' infinite-scroll-immediate='false' style="width:100%;overflow:visible">
                <el-row style="width:100%;border-bottom:1px solid #ccc;background:#fff;" v-for="(item,index) in dataList" :key='index' class="list-item" v-on:click.native="detail(item,3)">
                    <el-col :span="15">
                    ...
                    <el-col :span="9">
                        ...
                        <span style="float:right;margin-right:10px;font-size:12px;">
                            <p style="margin:7px 0;height:15px;width:65px;">{{item.createTime}}</p>
...
<script>
import { queryAllList } from "@/api/mobile";
export default {
  data() {
    return {
      inp: "",
  ...
  methods: {
    ...
    load() {
      ...
      if (this.arr.indexOf(this.current) < 0) {
        ...
        this.loading = true;
        setTimeout(() => {
          queryAllList(this.inp, this.old, this.current, this.size).then(
            res => {
              if (res.data.code == "200") {
