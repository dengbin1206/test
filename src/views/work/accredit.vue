    ...
    <basic-container>
        ...
        <avue-crud  ref='crud' :data='data' :option='option' :table-loading="loading" :page="page"   @selection-change="selectionChange"  @size-change="sizeChange"  @current-change="currentChange" :cell-style="cellStyle" :row-style="rowStyle">
            <template slot-scope="scope" slot="menu">
                  ...
                  <el-button type="text" size="small" icon="el-icon-delete" @click.stop="deltete(scope.row)" style="color:red">取消授权</el-button>
            </template>
...
import { myPowerList,deleteInstImPower,updateInstCopy,historyInfo } from "@/api/work/todo";
export default {
  components:{
      diaVisLog
  },
  ...
  methods: {
    ...
    deltete(row){
      ...
      this.$confirm('确认取消授权吗', '提示', {
          ...
          deleteInstImPower(params).then(res=>{
