    ...
    <basic-container>
        ...
        <avue-crud  ref='crud' :data='data' :option='option' :table-loading="loading" :page="page"   @selection-change="selectionChange"  @size-change="sizeChange"  @current-change="currentChange" :cell-style="cellStyle" :row-style="rowStyle">
            <template slot-scope="scope" slot="menu">
                  ...
                  <el-button type="text" size="small" icon="el-icon-view" @click.stop="edit(scope.row)">编辑</el-button>
                  <el-button type="text" size="small" icon="el-icon-delete" @click.stop="deltete(scope.row)" style="color:red">删除</el-button>
            </template>
...
import { myInstCopyList,deleteInstCopy,updateInstCopy,historyInfo } from "@/api/work/todo";
export default {
  components:{
      diaVisLog
  },
  ...
  methods: {
        ...
        extQueryList: [
          ...
          },
          {
            ...
            expressionList: [
              {
                operator: "EQ",
                lowerValue: val.delegateUserName,
                upperValue: ""
              }
            ]
          },
    ...
    deltete(row){
      ...
      this.$confirm('确认删除吗', '提示', {
          ...
          deleteInstCopy(params).then(res=>{
    ...
    //编辑
    edit(row){
      this.$refs.sonV.confirmProcess(this.dialog,row,1,'留言')
    },
  },
  created() {
    //发起部门
