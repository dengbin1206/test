  ...
  <div>
      ...
      <el-row>
        <el-col :span="11">
          <el-select
            ...
            clearable
            style="width:100%"
            :multiple="option.type =='multipleSelect' ? true : false"
            size="small"
            @change="changeLower"
    ...
    >
      <div style="margin-bottom:15px">
        <el-button type="primary" size="small" icon="el-icon-document-copy">粘贴</el-button>
      ...
      width="55">
        ...
        <el-table-column prop="lowerValue" label="值小" align="center">
          <template slot-scope="scope">
            <el-select v-model="scope.row.lowerValue" size="small" :placeholder="option.placeholder">
              <el-option
                v-for="item in option.list"
                :key="item.dictKey"
                :label="item.dictValue"
        ...
        <el-table-column prop="upperValue" label="值大" align="center">
          <template slot-scope="scope">
            <el-select v-model="scope.row.upperValue" size="small" :placeholder="option.placeholder">
              <el-option
                v-for="item in option.list"
                :key="item.dictKey"
                :label="item.dictValue"
...
export default {
  ...
  watch: {
    ...
    //        if(newVal !== oldVal){
    //          debugger
    //           this.$emit('childGetValue',newVal)
    //        }
    //   },
    //   deep:true,
    //   immediate:true
