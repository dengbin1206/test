<template>
  <div class="widget-form-container">
   <el-form
    ...
    <el-dialog width="70%" :visible.sync="dlgFormView" :before-close="handleClose" modal-append-to-body="false">
      <form-view v-if="dlgFormView" :data="itemOption" :option="formOption" :srcFormId="this.data.id"></form-view>
    </el-dialog>
    <el-dialog width="70%" :visible.sync="dlgChoice" :before-close="handleClose" modal-append-to-body="false">
      <form-view v-if="dlgChoice" :data="itemOption" :option="formOption" :srcFormId="this.data.id"></form-view>
    </el-dialog>
  </div>
  </div>
</template>
...
import { formById,formSave,formSearchNoPage } from "@/api/tool/analysis";
import WidgetGroup from "./widgetGroup";
import WidgetItem from "./widgetItem";
import getLocation from "../../utils/getlocation";
export default {
  name: "formView",
  props: ["data","option","srcFormId"],
  components: {
    WidgetGroup,
    WidgetItem,
    formView
  ...
  methods: {
      ...
      if(item.crud){
        if(this.data.currentRow){
          if(this.data.currentRow.ID){
            this.$set(this.itemOption, 'dataId', this.data.currentRow.ID);
          }
        }
      }
    ...
    searchTable(item) {
      ...
      this.options.column.forEach((cItem) => {
        if(cItem.type=='crud'){
          console.log("searchTablew:"+JSON.stringify(subItem));
