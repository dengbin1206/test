        ...
        const oRouter = {
          ...
          component(resolve) {
            ...
            if (first) {
              ...
              return
              // 判断是否为最终的页面视图
            } else if (path.indexOf('/tool/dynamic') != -1) {
              require(['../views/tool/dynamic'], resolve)
              return
            } else if(path.indexOf('/modelAnalysis/modelList') != -1){
              require([`../views/modelAnalysis/modelList`], resolve)
            } else if(path.indexOf('/modelAnalysis/sxForm') != -1){
              require([`../views/modelAnalysis/sxForm`], resolve)
            } else if(path.indexOf('/modelAnalysis/modelForm') != -1){
              require([`../views/modelAnalysis/modelForm`], resolve)
            } else if(path.indexOf('/formTranslate/index') != -1){
              require([`../views/formTranslate/index`], resolve)
            } else {
          ...
          children: !isChild ? (() => {
            if (first) {
              ...
              return [{
                component(resolve) {
                  if(path.indexOf('/tool/dynamic') != -1){
                    require([`../views/tool/dynamic`], resolve)
                  } else if(path.indexOf('/modelAnalysis/modelList') != -1){
                    require([`../views/modelAnalysis/modelList`], resolve)
                  } else if(path.indexOf('/modelAnalysis/sxForm') != -1){
