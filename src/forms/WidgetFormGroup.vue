  ...
  <div>
    ...
    <draggable class="widget-form-group__body"
      ...
      <template v-for="(item, groupIndex) in column.children.column">
        ...
        <!-- <el-col v-else-if="column.type == 'tabs'" :key="groupIndex"
                    :md="column.span || 12"
                    :xs="24"
                    :offset="column.offset || 0">
                <el-tabs type="border-card"  
                :tab-position="column.params.option.position" :class="{active: selectWidget.prop == column.prop}">
                  <el-tab-pane :label="tab.label" v-for="(tab,i) in column.params.option.column" :key="i">
                    <div class="widget-form-group"
                          style="min-height: 100px"
                         @click="handleWidgetTableSelect(index)">
                      <widget-form-tabs
...
import WidgetFormTabs from './WidgetFormTabs'
import draggable from 'vuedraggable'

export default {
  name: 'widget-form-group',
  props: ['data', 'column', 'select', 'index'],
  components: { WidgetFormItem, WidgetFormTable, WidgetFormTabs, draggable },
  data () {
    return {
