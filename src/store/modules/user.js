...
import { getTopMenu, getRoutes, getRoutesAll } from '@/api/system/function'
import { mobileLogin } from "@/api/mobileApi/flow";
...
function addPath(ele, first) {
  ...
  if (!isChild && first && !isURL(ele[propsDefault.path])) {
    ...
    ele[propsDefault.children].forEach(child => {
      //ele[propsDefault.path] = child[propsDefault.path] + '/index'
      addPath(child);
    })
  }
...
const user = {
  ...
  actions: {
    ...
    //根据用户名登录 { 手机端 }
    MobileLoginByUserName({ commit }, userInfo) {
      return new Promise((resolve, reject) => {
        mobileLogin(userInfo.tenantId, userInfo.username, md5(userInfo.password), userInfo.type, userInfo.key, userInfo.code).then(res => {
          const data = res.data;
          if (data.error_description) {
            Message({
              message: data.error_description,
    ...
    GetMenuAll({ commit, dispatch }) {
      return new Promise(resolve => {
        getRoutesAll().then((res) => {
          const data = res.data.data
          let menuAll = deepClone(data);
          menuAll.forEach(ele => {
            addPath(ele, true);
          });
